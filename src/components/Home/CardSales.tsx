...
export default function CardSales({ url, name, category_id, product_id, price,category_name }: productInfo) {
    ...
    return (
        <div className='w-full sm:w-1/2  xl:w-1/3  py-3 px-2  md:hover:scale-110 md:transition-all md:duration-200'>
            {
                <Link href={`/products/${category_name}/${product_id}`} className='w-full'>
                    <div className=' w-full bg-white border rounded-lg shadow-md shadow-[#8a8a8a] md:hover:bg-green-100 md:transition-all md:duration-200'>
                        <div className='py-1 px-1 '>
                            <img src={url} className='rounded-lg h-96 xl:h-72 w-full object-cover ' alt={"category"} />
                        </div>
